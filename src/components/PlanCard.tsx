      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          {showAirFiberToggle && categoryFilter === 'all' && (
            <div className="mt-6 inline-flex p-1.5 rounded-2xl bg-surface border border-edge shadow-inner">
              <button
                type="button"
          ...
          <div className="mt-8 flex flex-col items-center">
            <div className="inline-flex p-1.5 rounded-2xl bg-surface-2 border border-edge shadow-inner max-w-full overflow-x-auto">
              {(
                [
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-semibold whitespace-nowrap transition-all duration-300 ${
                      isSelected
                        ...
                        : 'text-ink-faint hover:text-ink-strong hover:bg-slate-200 dark:hover:bg-white/[0.04]'
                    }`}
                  >
                    {cycle.label}
                  </button>
            ...
            {billingCycle !== 'monthly' && (
              <motion.div
                ...
                className="mt-3 inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg bg-surface border border-amber-500/20 text-amber-300 text-xs"
              >
        ...
        <div className="mt-10 text-center">
          <p className="text-xs text-ink-faint flex items-center justify-center gap-1.5">
  ...
  return (
    <motion.div
      ...
      className={`relative rounded-3xl p-6 sm:p-7 flex flex-col justify-between transition-all duration-300 ${
        isRecommended
          ? 'z-10 lg:-translate-y-2 bg-gradient-to-b from-[#dbeafe] via-[#e0f2fe] to-[#f0f9ff] dark:from-[#0d2248] dark:via-[#091530] dark:to-[#070c1d] border-2 border-[#00E5FF] shadow-[0_0_45px_rgba(0,229,255,0.24)]'
          : isFastest
          ? 'bg-gradient-to-b from-[#ede9fe] to-[#eef2ff] dark:from-[#141233] dark:to-[#0B1224] border border-[#6C63FF]/50 shadow-xl shadow-[#6C63FF]/10 hover:border-[#6C63FF]'
          : 'bg-surface/85 backdrop-blur-xl border border-edge hover:border-edge-strong shadow-lg shadow-black/40'
      }`}
    >
      {/* Plan Header Badge */}
      {plan.badge && (
        <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 whitespace-nowrap">
          ...
          >
            {isRecommended && <span className="w-1.5 h-1.5 rounded-full bg-app animate-pulse" />}
      ...
      <div>
        <div className="flex items-baseline justify-between mb-3">
          <h3 className="text-xl font-extrabold text-ink-strong font-['Manrope']">
            {plan.name}
          </h3>
          <span className={`text-[11px] font-semibold px-2.5 py-0.5 rounded-full ${
            isRecommended
              ? 'bg-[#00E5FF]/15 text-[#00E5FF] border border-[#00E5FF]/30'
              : 'bg-slate-100 dark:bg-white/[0.06] text-ink-soft'
          }`}>
            {plan.category === 'fiber' ? 'Fiber FTTH' : 'Air-Fiber'}
          </span>
        </div>

        <p className="text-xs text-ink-faint min-h-[34px] leading-relaxed mb-6">
          {plan.description}
        </p>
        ...
        <div className={`py-4 px-4 rounded-2xl mb-6 flex items-center justify-between border ${
          isRecommended
            ? 'bg-app/90 border-[#00E5FF]/30'
            : 'bg-app/70 border-edge'
        }`}>
          <div>
            <div className="text-3xl font-extrabold text-ink-strong font-['Manrope'] tracking-tight flex items-baseline gap-1">
          ...
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
            isRecommended ? 'bg-[#00E5FF]/20 text-[#00E5FF]' : 'bg-slate-100 dark:bg-white/[0.06] text-[#00E5FF]'
          }`}>
        ...
        <div className="mb-6">
          <div className="flex items-baseline gap-1">
            <span className="text-3xl sm:text-4xl font-extrabold text-ink-strong font-['Manrope']">
              ₹{cycleTotal}
            </span>
        ...
        <div className="space-y-3 pt-4 border-t border-edge">
          ...
          {plan.features.map((feat, idx) => (
            <div key={idx} className="flex items-center gap-2.5 text-xs text-ink-soft">
        ...
        <Link
          ...
          className={`w-full flex items-center justify-center gap-2 py-3.5 px-4 rounded-2xl text-xs font-bold transition-all duration-200 active:scale-95 ${
            isRecommended
              ...
              : 'bg-slate-100 dark:bg-white/[0.08] hover:bg-slate-200 dark:hover:bg-white/[0.14] text-ink-strong border border-edge-strong'
          }`}
        >
